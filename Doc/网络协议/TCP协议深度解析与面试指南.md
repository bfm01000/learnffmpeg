...
- 面试复习：先掌握三次握手/四次挥手、可靠传输、滑动窗口、拥塞控制、Nagle、Keepalive 和 TIME_WAIT。
- 深入学习：重点看 TCP 与实时音视频的矛盾：可靠有序带来队头阻塞，重传会放大延迟，发送缓冲区可能隐藏排队。
- 工程落点：RTMP、HTTP-FLV、HLS 都离不开 TCP；弱网卡顿排查要能结合抓包、内核参数、发送队列和业务丢帧策略。
