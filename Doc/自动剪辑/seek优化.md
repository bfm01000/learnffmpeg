...
### 7.2 关键帧索引表 (Index Table) 与终极演进（面试高光应对）

**潜在缺陷（性能倒挂陷阱）**：
目前的 Smart Seek 依赖 `1500ms` 的固定经验阈值。假设当前解码到 `100ms`，目标是 `1400ms`（距离 1300ms < 1500ms，触发 Smart Seek）。但如果视频在 `1000ms` 处**刚好有一个关键帧**，那么：
- 走 Smart Seek：需要顺序解码 `100ms -> 1400ms`（解 1300ms 数据）。
- 走老逻辑：直接 Seek 到 `1000ms` 关键帧，解码 `1000ms -> 1400ms`（仅解 400ms 数据）。
此时，固定阈值反而导致了**性能倒挂**。

**面试应对方案（高光话术）**：
在面试中，建议主动向面试官承认这个固定阈值的缺陷，并抛出终极解决方案：
> “目前的固定阈值在跨越关键帧时可能会出现性能倒挂。为了彻底解决这个问题，我的终极方案是**直接解析 MP4 的 `stss`（关键帧索引表）box，拿到所有关键帧的时间戳数组**。
> 在触发 Seek 前进行精准判断：**检查当前位置到目标位置之间，是否存在关键帧？**
> - 如果**有**关键帧：说明跨越了 GOP 边界，直接走老逻辑 Seek 到那个关键帧，避免多余的前向解码。
> - 如果**没有**关键帧：说明完全在同一个 GOP 内，此时才安全地走 Smart Seek 前向追赶。
> 这样就能实现 100% 精准的策略路由，彻底消除性能倒挂的风险。”
