...
*   **CPU 喜欢 Linear（线性布局）**：一行接着一行连续存储，方便 CPU 的 Cache Line 顺序预取。
*   **GPU 喜欢 Tiled（块状/交错布局）**：把相邻的像素放在物理内存相邻的位置（如 Z 字形或 Morton 曲线），这样在做 2D 纹理采样（如双线性插值）时缓存命中率最高。
*   **VPU（硬件编解码器）也极度依赖 Tiled 布局**：视频编码算法（如 H.264/H.265）是基于 16x16 或 32x32 的**宏块（Macroblock）**进行运动估计和预测的。Tiled 布局能保证宏块数据在物理内存上的连续性。此外，VPU 还原生支持读取 GPU 生成的**硬件无损压缩块（如 AFBC/UBWC）**，从而极大节省内存带宽。
