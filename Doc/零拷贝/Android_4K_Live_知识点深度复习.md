...
- 面试复习：先掌握 4K 直播性能优化的核心矛盾：CPU 读回、GPU 同步、格式转换、队列背压和编码器输入通路。
- 深入学习：重点看 `glReadPixels` 优化、PBO/Fence、AHardwareBuffer usage、RHI 抽象代价和 Android/iOS 零拷贝对比。
- 工程落点：这篇偏项目复盘，适合把底层知识组织成“我如何定位和优化 4K 直播性能”的面试故事。
