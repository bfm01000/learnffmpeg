...
> **阅读建议**：先掌握 [[01-多线程与锁]] 的 `mutex`/`condition_variable`，以及 [[02-原子操作与内存序]] 的 `std::atomic` 与 `memory_order`，再看本篇无锁队列。
...
`volatile` 只限制编译器优化，不保证原子性，也不保证跨线程内存可见性。并发同步应该使用 `std::atomic`（完整论证见 [[13-语言基础关键字]]，`memory_order` 与 CAS 见 [[02-原子操作与内存序]]）。
