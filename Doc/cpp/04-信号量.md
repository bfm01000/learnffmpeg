...
- 面试复习：先掌握 P/V、`counting_semaphore`、二值信号量和条件变量的差异。
- 深入学习：重点看它如何表达“资源数量”而不是“互斥访问”，以及超时等待、停止通知和异常退出。
- 音视频落点：信号量适合限制帧池、编码任务、下载缓冲和并发滤镜数，常与有界队列一起形成背压。
> **适用方向**：Android / iOS / FFmpeg 音视频开发，C++ 多线程，Pipeline / 线程池 / 连接池  
> **难度**：⭐⭐⭐  
> **预计阅读**：速记 8 分钟｜全文 25 分钟  
> **关联文档**：[01-多线程与锁](01-多线程与锁.md)（mutex、条件变量）、[03-无锁队列](03-无锁队列.md)（SPSC 队列）、[02-原子操作与内存序](02-原子操作与内存序.md)（atomic）
...
>
> 📎 深入对比见下方 [#二-B、信号量 vs mutex-条件变量：深入对比](#二-b信号量-vs-mutex-条件变量深入对比)，包含决策树、性能差异、组合模式。
