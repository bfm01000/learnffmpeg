...
>
> 相关：`const` 全局变量的存储位置见 [[05-内存管理与对象生命周期]]；模板元编程见 [[14-模板与泛型编程]]；`consteval`/`constinit` 也在 [[17-C++20与协程]] 提到。
...
> 现代建议：**优先 `constexpr`/`consteval` 函数**做编译期计算，可读性远好于模板递归；模板元编程主要用于类型层面的操作（见 [[14-模板与泛型编程]] 的 type_traits / CRTP）。
