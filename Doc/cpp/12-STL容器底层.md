...
>
> 相关：扩容触发的拷贝/移动见 [[07-移动语义与右值引用]]；内存分配与对齐见 [[05-内存管理与对象生命周期]]。
...
> **为什么移动构造要写 `noexcept`？** 扩容搬元素时，若元素的移动构造**不保证 noexcept**，`vector` 为了异常安全（搬到一半抛异常无法回滚）会**退化成拷贝**。给移动构造/移动赋值加 `noexcept` 才能让扩容走移动、避免昂贵拷贝。详见 [[07-移动语义与右值引用]]。
...
`push_back`/`insert` 接收**已构造好的对象**，再拷贝/移动进容器；`emplace_back`/`emplace` 接收**构造参数**，在容器内存上**原地构造**（完美转发，见 [[07-移动语义与右值引用]]），省一次临时对象的构造与移动。
