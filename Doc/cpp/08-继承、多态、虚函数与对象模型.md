...
- 面试复习：先掌握 vptr/vtable、虚析构、override/final、对象切片、多继承、虚继承和构造析构期虚调用。
- 深入学习：重点看对象模型的内存布局与 ABI，理解动态分发的成本和边界。
- 音视频落点：播放器 source/decoder/renderer/filter 抽象常用多态，但热路径不宜滥用层层虚调用。
