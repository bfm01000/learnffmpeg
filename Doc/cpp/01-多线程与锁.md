...
## 八、atomic 和锁是什么关系？（摘要，详见 [[02-原子操作与内存序]]）

> 原子操作、`memory_order`、CAS、`volatile` vs atomic 的完整内容已独立成篇，见 [[02-原子操作与内存序]] 与 [[13-语言基础关键字]]。这里只保留与“锁”对比的速记。
...
- **atomic 适合单变量、mutex 适合复合不变量** 的完整对比、`memory_order` 详解、CAS → [[02-原子操作与内存序]]
- **`volatile` 为什么不能替代 atomic/锁**（含与 Java volatile 的区别、双重检查锁定）→ [[13-语言基础关键字]]
...
- [[02-原子操作与内存序]]
- [[06-智能指针与资源管理]]
- [[13-语言基础关键字]]（volatile / explicit / inline）
