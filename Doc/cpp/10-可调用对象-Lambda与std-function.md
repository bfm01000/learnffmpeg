...
>
> 相关：函数调用开销与内联见 [[13-语言基础关键字]]；`std::function` 的虚表机制见 [[08-继承、多态、虚函数与对象模型]]；按引用捕获导致的悬垂问题与生命周期管理见 [[06-智能指针与资源管理]]。
...
> `enable_shared_from_this` 的原理见 [[06-智能指针与资源管理]]。
...
- 不捕获（`[]`）：通常 1 字节（空类大小为 1）。
- 有捕获：所有捕获变量之和（考虑内存对齐，见 [[05-内存管理与对象生命周期]]）。
...
- **线程池任务队列**：`std::queue<std::function<void()>>` 配合互斥锁/条件变量（见 [[01-多线程与锁]]），把解封装、解码、缩放等任务投递进去。
