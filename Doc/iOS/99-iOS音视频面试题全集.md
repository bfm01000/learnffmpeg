...
- 面试复习：这是 iOS 音视频集中题库，适合考前按采集、编码、解码、渲染、音频、推流、性能逐块查漏补缺。
- 深入学习：每道题都应回链到对应专题，尤其是 `VideoToolbox`、AVCC/Annex-B、Metal 零拷贝和 AudioSession。
- 使用方法：中高级回答要讲清 API 生命周期、底层 buffer、线程模型、异常恢复和性能验证。
