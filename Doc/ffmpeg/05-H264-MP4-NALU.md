...
为什么要防 `00 00 00` / `00 00 02` / `00 00 03`？`00 00 00` 是 4 字节起始码的前缀，如果数据里出现 `00 00 00 01 xx`，解析器可能把真实起始码吃掉；防 `0x02` 是预留（某些扩展用 `00 00 02` 做保留标记）；防 `0x03` 本身则是避免"上次插入的防竞争字节被当成新数据的 `00 00 03` 再转义一次"。
