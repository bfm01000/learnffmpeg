...
- 播放 PCM 必须知道哪 5 个参数？少任何一个会怎样？
- 为什么 FFmpeg 解 AAC 默认输出 `FLTP`？为什么直接送 SDL 会刺耳？
- `nb_samples` 是总样本数还是每个声道的样本数？
- 重采样输出 buffer 大小为什么不能简单按比例算？正确公式是什么？
- 为什么音频回调里不能加锁、不能打日志？
- AAC 编码为什么需要 `AVAudioFifo`？
- 音视频同步为什么以音频为主时钟？
- AEC 为什么需要远端播放参考信号？
- Jitter Buffer 解决什么问题？太大太小各有什么代价？
- Seek 之后为什么容易音画不同步？要清哪几个东西？
