...
| 项 | 说明 |
|---|---|
| 面试位置 | 音频专项长文：比 04/18 更面向工程排查和面试表达。 |
| 先背什么 | PCM、重采样、AAC/Opus、同步、回声、jitter buffer 都要能用口语讲。 |
| 深入怎么学 | 适合作为音频线总复习，再回到 04/18/24 查具体 API。 |
| 关联阅读 | 04、18、24 |
...
### Q11：I 帧很大、P 帧很小会造成什么问题？怎么解决？

可以这样答：

> 核心问题是码率尖峰和质量波动。I 帧过大会导致：① 网络传输时瞬时码率飙升，挤占后续 P 帧的发送窗口，造成周期性卡顿，且大 I 帧拆成的大量网络包中任意一个丢失都会导致整帧损坏；② VBV 缓冲区被 I 帧掏空，后续 P 帧被迫使用极高 QP 编码，GOP 尾部画面质量严重劣化，形成周期性的"清晰→模糊→清晰"呼吸效应；③ 解码耗时严重不均，I 帧解码可能超过帧间隔预算导致掉帧；④ I 帧大小波动会误导 ABR 算法错误降档，并拖慢首屏和 seek。
>
> 解决方式：① 启用 VBV 码率控制（`-maxrate` + `-bufsize`），用 HRD 缓冲区模型硬约束 I 帧上限，这是直播和实时通信的必选手段；② 降低 `ip_ratio`（如从 1.40 调整到 1.10~1.20），缩小 I/P 帧之间的 QP 差距；③ 使用 Intra Refresh 代替周期性完整 I 帧，把 intra 块分散到连续多帧，消除码率尖峰，尤其适合 WebRTC 等低延迟场景；④ 适当缩短 GOP 长度，让 I 帧更频繁但更小；⑤ VOD 场景使用两遍编码（2-pass），基于全视频复杂度统计做全局最优码率分配；⑥ 开启 lookahead 和 mb-tree，让编码器提前感知后续帧的复杂度，优化码率的时间分配。
>
> 注意：已编码完成的视频无法通过修改 metadata 调整 I/P 帧比例，只能重新转码。
