...
> 因为 NVIDIA 在**消费级的 GeForce 卡驱动里限制了同时能开的 NVENC 编码会话数**，常见是 3 到 5 路，数据中心卡（T4、L4 这些）才放开。云转码是按"一张卡同时编多少路"算成本的，你要是用游戏卡，开到第 4、第 5 路就直接起不来了。所以这是个选型硬约束——做转码集群必须上数据中心卡，不是说性能不够，是驱动给你卡住了路数。
...
```
软解码:  码流(RAM) → CPU 跑 x264 解码 → 原始帧(RAM) → 应用直接读
...
> **消费级 GeForce 卡的驱动，限制了同时进行的 NVENC 编码会话（session）数量**（历史上常见的限制是 同时 3~5 路，随驱动版本调整）。**专业卡 / 数据中心卡（Quadro/RTX A 系、Tesla/A 系列、L4/L40 等）解除或大幅放宽这个限制。**
...
| 新 preset | 含义 | 对标旧名（已废弃别用） |
|---|---|---|
| P1 | 最快 | `hp`（high performance） |
| P4 | 中间默认档 | `default`/`medium` |
| P7 | 最慢最高质量 | `hq`（high quality）/`slow` |

旧的 `default / hp / hq / llhq / llhp / lossless` 这套命名**已废弃**，新代码一律用 `-preset p1..p7` 配 `-tune`。
...
| `cbr_ld_hq` | 低延迟高质量 CBR | 直播/云游戏低延迟 |
...
- **怎么用**：游戏在 GPU 上渲染 → 渲染结果（已在 VRAM）通过 interop 直接喂 NVENC → `-tune ull` 超低延迟 → `cbr_ld_hq` 码控 → 关 B 帧、关 look-ahead。
...
> `-tune ull`（超低延迟）+ `-rc cbr_ld_hq` + 关 B 帧（`-bf 0`）+ 关/压小 `-rc-lookahead`。本质和 06 的 `zerolatency` 一样——干掉「要等未来帧」的手段（§6.2）。
