...
> 
> **Q4：讲一下对 isa 指针的理解？（高频追问：如果调用父类方法，只靠 isa 能找到吗？）**
...
> **但是，如果只靠 `isa` 是绝对找不到父类方法的！** 比如 B 继承 A，B 调用 A 的方法时，顺着 B 实例的 `isa` 只能找到 B 的类对象。如果在 B 的类对象里找不到，Runtime 就会顺着 B 类对象内部的 **`superclass` 指针**去 A 的类对象里找，一直找到根类（NSObject）。
> **如果是调用父类的【类方法】呢？原理是一模一样的！** 假设调用 `[B classMethod]`，Runtime 会先顺着 B 类的 `isa` 找到 **B 的元类**（这是起点）。如果在 B 的元类里找不到，就会顺着 B 元类内部的 `superclass` 指针，找到 **A 的元类**，在 A 的元类里查找这个类方法。
> 一句话总结：**`isa` 决定了第一步去哪找（实例找类，类找元类），`superclass` 决定了找不到时顺着哪条线往上找。**”
> 
