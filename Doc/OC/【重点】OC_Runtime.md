...
> 🗣️ **口语化回答**：“Runtime 就是 OC 的运行时机制，让 OC 具备了动态特性。底层是用 C/C++ 和汇编实现的一套 API。比如 `[obj doSomething]`，编译时还没确定具体方法实现，而是在运行时通过 `objc_msgSend` 去实际查找和调用。正是有了 Runtime，才让我们可以动态创建类、添加/替换方法（Method Swizzling）等。”
>
