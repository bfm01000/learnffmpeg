...
> 
> **Q4：讲一下对 isa 指针的理解？**
> 🗣️ **口语化回答**：“在 OC 里，每个对象内部都有一个 `isa` 指针。实例对象的 `isa` 指向它的类对象（Class），这样当实例调用方法时，就能顺着 `isa` 找到类对象里的方法列表。而类对象本身也是个对象，它的 `isa` 指向元类（Meta Class），用来查找类方法。元类的 `isa` 最终都指向根元类（通常是 NSObject 的元类），根元类的 `isa` 指向它自己，从而形成一个闭环。”
> 
