...
> 
> **Q1：简单介绍一下什么是 OC 的 Runtime？**
> 🗣️ **口语化回答**：“Runtime 其实就是 OC 的运行时机制，它是 OC 能够成为动态语言的基石。底层是用 C、C++ 和汇编实现的一套 API。我们在写 OC 代码的时候，比如调用一个方法 `[obj doSomething]`，在编译阶段其实只是确定了要发送消息，真正找到这个方法并执行，是推迟到运行阶段通过 `objc_msgSend` 来完成的。正是因为有了 Runtime，我们才能在程序运行的时候动态地创建类、添加方法、替换方法实现（Method Swizzling）等等。”
> 
> **Q2：讲一下 OC 中方法调用的底层机制（消息发送机制）？**
> 🗣️ **口语化回答**：“OC 的方法调用本质上是发送消息。当我们调用一个方法时，编译器会把它转成 `objc_msgSend(obj, @selector(method))`。底层主要分三大阶段：
> 第一阶段是**消息发送**，会先去对象的类对象里的 `cache`（缓存）找，找不到就去 `method_list`（方法列表）找，再找不到就顺着 `superclass` 去父类找。
> 如果一直找到根类都没找到，就会进入第二阶段**动态方法解析**，系统会给你一次机会，问你要不要动态添加一个方法。
> 如果还不处理，就会进入第三阶段**消息转发**，你可以把这个消息转给别的对象来处理。如果这三步都走完了还是没处理，程序就会 Crash，报 `unrecognized selector` 错误。”
> 
> **Q3：什么是 Method Swizzling（黑魔法）？你在项目里用它做过什么？**
> 🗣️ **口语化回答**：“Method Swizzling 本质上就是利用 Runtime 动态交换两个方法的实现（IMP）。每个类都有一个方法列表，里面存着 SEL（方法名）和 IMP（函数指针）的映射关系。Swizzling 就是把这两个 SEL 指向的 IMP 交换一下。在项目里，我们通常用它来做 AOP（面向切面编程），比如无侵入地统计页面的 PV/UV（交换 `UIViewController` 的 `viewWillAppear`），或者做全局的防 Crash 处理（比如拦截 `NSArray` 的 `objectAtIndex:` 防止越界崩溃）。不过使用时一定要放在 `+load` 方法的 `dispatch_once` 里，保证只交换一次，否则容易出大问题。”
> 
> **Q4：讲一下对 isa 指针的理解？（高频追问：如果调用父类方法，只靠 isa 能找到吗？具体是怎么找的？）**
> 🗣️ **口语化回答**：“在 OC 里，`isa` 指针主要负责‘找类型’，而 `superclass` 指针负责‘找继承’，它们俩是配合工作的。
> 首先说 `isa`：实例对象的 `isa` 指向类对象，类对象的 `isa` 指向元类，元类的 `isa` 最终指向根元类形成闭环。**`isa` 的作用是决定了方法查找的【起点】**（实例方法去类对象找，类方法去元类找）。
> 
> **具体在每个节点是怎么找的呢？（结合 cache 和 method_list）**
> 当顺着 `isa` 来到一个类（或元类）时，Runtime 并不是上来就死磕方法列表：
> 1. **快速查找（cache）**：先去类对象内部的 `cache`（一个哈希表）里找。这一步是用纯汇编实现的，速度极快。如果之前调用过，直接命中拿到 IMP 执行。
> 2. **慢速查找（method_list）**：如果 cache 里没有，才会进入 C/C++ 层面，去类的 `class_rw_t` 里的 `method_list` 中查找（已排序的列表用二分查找，未排序的用遍历）。如果找到了，会把这个方法塞进当前类的 `cache` 里，方便下次秒查。
> 
> **如果当前类找不到，怎么找父类？**
> 这时候 `isa` 就帮不上忙了。比如 B 继承 A，B 调用 A 的方法，在 B 的类对象里（cache 和 method_list）都没找到。Runtime 就会顺着 B 类对象内部的 **`superclass` 指针**来到 A 的类对象。然后在 A 的类对象里，**再次重复** `cache` -> `method_list` 的查找流程。一直往上找，直到根类（NSObject）。
> **如果是调用父类的【类方法】**，原理一模一样：顺着 B 类的 `isa` 找到 **B 的元类**，找不到就顺着 `superclass` 找到 **A 的元类**，在里面查 cache 和 method_list。
> 
> 一句话总结：**`isa` 决定了第一步去哪找，`superclass` 决定了找不到时顺着哪条线往上找；而每到一个节点，都是先查 `cache`，再查 `method_list`。**”
> 
> **Q5：Category（分类）为什么不能直接添加成员变量？**
> 🗣️ **口语化回答**：“因为一个类的内存布局在编译期就已经死死定好了。对象的实例变量在底层其实就是一段连续的内存偏移量。运行的时候，这块内存早就分配完了，你不可能再硬生生地改变它的大小。如果非要在分类里加变量，我们通常的绕过方案是利用 Runtime 的**关联对象（Associated Object）**技术，本质上是系统在底层维护了一个全局的 Hash 表来存这些额外的数据，并没有改变对象本身的内存结构。”
