...
- 面试复习：先掌握 RTMP 在直播推流中的位置，能按 TCP 连接、握手、命令交互、元数据、音视频 Message/Chunk 讲完整流程。
- 深入学习：重点看 H.264/AAC 如何进入 FLV/RTMP、timestamp 如何处理、弱网为何堆积以及 FFmpeg 推流 API 怎么落地。
- 职责边界：这篇偏直播链路和面试口述；字段级 Chunk/AMF/服务端 parser 深挖看 [../网络协议/RTMP协议深度解析与面试指南.md](../网络协议/RTMP协议深度解析与面试指南.md)。
