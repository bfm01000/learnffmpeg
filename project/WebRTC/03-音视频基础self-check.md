...
以 `4×4` 图像为例，先看 Y 平面——每像素一个 Y，16 个像素共 16 个 Y 值：
...
色度是 4:2:0 下采样：每 `2×2` 的 Y 矩阵共用一组 UV。所以 `4×4` 的 Y 对应 `2×2` 组 UV：

```text
【UV 平面】 (U 和 V 紧挨着交错排列)
NV12:                        NV21:
U0 V0   U1 V1                V0 U0   V1 U1
U2 V2   U3 V3                V2 U2   V3 U3
```

> **关键理解**：Y0, Y1, Y4, Y5 这 4 个亮度像素，共同使用 U0, V0 这组色度数据；Y2, Y3, Y6, Y7 共用 U1, V1；以此类推。

**NV12 的顺序是 U 在前、V 在后**（`U,V,U,V...`），这也是硬件编解码器的首选格式；**NV21 的顺序反过来，V 在前、U 在后**（`V,U,V,U...`），是 Android 摄像头常用的默认格式。

对应到 `AVFrame`，I420 会用到 `data[0] / data[1] / data[2]` 三个指针；NV12/NV21 通常只用 `data[0]` 指向 Y、`data[1]` 指向交错的 UV/VU，`data[2]` 不再代表独立 V 平面。访问第 `(x, y)` 个亮度像素看 `data[0] + y * linesize[0] + x`；访问色度时要先把坐标缩半成 `(x/2, y/2)`，NV12 的 U/V 地址是 `data[1] + (y/2) * linesize[1] + (x/2) * 2` 和后一个字节，NV21 则 V/U 顺序相反。硬件编解码器偏爱 NV12，是因为双平面对 DMA、SIMD 和 GPU 纹理采样都更友好——UV 交错存放，一次读取就是一组完整的色度数据。
...
**为什么 HLS 每个分片都要自包含？** 想象一个场景：用户在 B 站看直播回放，拖进度条跳到第 30 分钟。播放器向 CDN 请求的是**第 30 分钟对应的那个 .ts 分片**，不会去下载第 1 分钟的分片。如果这个分片里没有 SPS/PPS/PAT/PMT，解码器拿到数据后根本不知道分辨率多少、用哪个编码档次、流里有几条音视频轨道——**永远黑屏**。同样的场景也发生在 CDN 节点切换、网络断开重连时——播放器总是从"当前时间点对应的那个分片"重新开始，而不是从头下。
...
**自检**：你能口述清楚吗？[ ] Y / [ ] N
**已有文档**：`Doc/ffmpeg/19-HLS详解.md` §3.4（自包含切片专题）
...
**适用场景**：
- **本地播放器**（mpv/VLC/IINA）——放任何有声音的视频文件
- **在线点播**（YouTube/Netflix/B 站拖动进度条后恢复播放）——seek 后第一帧 pts 需要重新和 audio_clock 对齐
- **直播拉流观众端**（HLS/FLV 播放器）——网络抖动丢帧时优先保音频连续
- **硬件播放盒/机顶盒**——HDMI 输出时音频走 I2S/SPDIF、视频走 HDMI 帧同步信号，音频时钟是物理上的主
...
**适用场景**：
- **无音轨/静音监控**——安防摄像头画面、延时摄影、工业相机录制，没有音频可追，视频本身就是唯一时钟
- **视频编辑软件的 timeline 预览**（Premiere/Final Cut/DaVinci Resolve）——timeline 以视频帧为基准（"第 42 帧"），音频必须拉伸/压缩适配剪辑节奏；JKL 快进/慢放时音频也跟着加速/减速
- **屏幕录制/教学视频录制**——帧率可能不固定（VFR），以实际帧的时间戳为基准，音频录制后对齐
- **游戏录像/回放**——游戏引擎以固定帧步进（tick），音频要追游戏帧时钟
- **视频会议"共享屏幕"模式**——屏幕共享流优先保证清晰流畅，音频不是主角

**关键区别**：视频追音频是"视频跳帧"，音频追视频是"音频变时长"——前者至少不改变音频质量，后者可能引入音频失真。
...
**适用场景**：
- **WebRTC 1v1 通话 / 多人会议**——两端时钟独立，各自以 NTP 同步过的墙钟为基准。音频走 NetEQ（时域伸缩 + PLC），视频走 JitterBuffer，各自独立向墙钟对齐，互不追。原因：两端是不同机器，对方的音频/视频可能来自不同 SSRC，追任何一路都破坏其他路
- **直播连麦（SFU 多路混流）**——主播 A、嘉宾 B、嘉宾 C 各有独立时钟，SFU 服务器以 NTP 墙钟为参考对齐多路流后混屏/混音
- **SRT/RIST 等低延迟传输**——接收端解码后用墙钟做 master，双方不需要互相追
- **需要多流精确同步的场景**——例如体育赛事多机位回放，所有机位以同一个外部时钟（如 GPS PTP/IEEE 1588 时钟）对齐后再混切

**面试常追问**：外部时钟模式下怎么处理时钟漂移？答：双方各自用 NTP 定期同步、或 RTCP SR（Sender Report）里的 NTP timestamp 做双向时钟校准。
