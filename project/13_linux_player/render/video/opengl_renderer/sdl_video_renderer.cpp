...
int SDLVideoRenderer::init(const RenderConfig& cfg) {
  if (SDL_InitSubSystem(SDL_INIT_VIDEO) < 0) { fprintf(stderr,"SDL video init: %s\n",SDL_GetError()); return -1; }

  int w=cfg.width>0?cfg.width:1280, h=cfg.height>0?cfg.height:720;
  m_winW=w; m_winH=h;
  m_window=SDL_CreateWindow(cfg.title?cfg.title:"Player SDK", SDL_WINDOWPOS_UNDEFINED,SDL_WINDOWPOS_UNDEFINED,w,h,SDL_WINDOW_RESIZABLE);
  if(!m_window) { fprintf(stderr,"SDL window: %s\n",SDL_GetError()); return -1; }
  m_renderer=SDL_CreateRenderer(m_window,-1,SDL_RENDERER_ACCELERATED);
  if(!m_renderer) { fprintf(stderr,"SDL renderer: %s\n",SDL_GetError()); return -1; }
  return 0;
}
